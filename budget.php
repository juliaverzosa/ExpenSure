<?php  
  class Budget extends Base {
    ...
    public function budget_validity_checker($UserID){
      $stmt = $this->pdo->prepare("SELECT EXTRACT(MONTH FROM Record_Date) AS mon FROM budget WHERE UserID = :user");
    ...
    public function updatebudget($UserID, $budget) {
      $stmt = $this->pdo->prepare("UPDATE budget SET Budget = :amount, Record_Date = CURRENT_TIMESTAMP() WHERE UserID = :user");
      $stmt->bindParam(":user", $UserID, PDO::PARAM_INT);
      $stmt->bindParam(":amount", $budget, PDO::PARAM_INT);
      $stmt->execute();
    }
